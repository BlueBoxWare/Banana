...
function parseFilename(filename) {
  ...
  let updatePart =
    nameParts.length >= 3 ? nameParts[nameParts.length - 2] : null;
  let positionPart =
    nameParts.length >= 4 ? nameParts[nameParts.length - 3] : null;
  ...
  if (positionPart !== null && positionPart.length >= 1) {
    ...
    if (BOXES.hasOwnProperty(box) && /^\d*$/.test(position)) {
      ...
      if (position.length > 0) settings.position = parseInt(position, 10);
    }
  }
...
function parseLine(lineString) {
  ...
  if (separatorIndex >= 0) {
    let attributes = [];
    try {
      attributes = GLib.shell_parse_argv(
        lineString.substring(separatorIndex + 1)
      )[1];
    } catch (error) {
    ...
    for (let i = 0; i < attributes.length; i++) {
      ...
      if (assignmentIndex >= 0) {
        ...
        if (name.length > 0 && value.length > 0) line[name] = value;
      }
    }
  ...
  if (line.hasOwnProperty("color"))
    markupAttributes.push(
      "color='" + GLib.markup_escape_text(line.color, -1) + "'"
    );

  if (line.hasOwnProperty("font"))
    markupAttributes.push(
      "font_family='" + GLib.markup_escape_text(line.font, -1) + "'"
    );
  ...
  if (line.hasOwnProperty("size")) {
    ...
    let fontSize = isNaN(pointSize)
      ? line.size
      : Math.round(1024 * pointSize).toString();
    markupAttributes.push(
      "font_size='" + GLib.markup_escape_text(fontSize, -1) + "'"
    );
  }
  ...
  if (!line.hasOwnProperty("emojize") || line.emojize !== "false") {
    line.markup = line.markup.replace(/:([\w+-]+):/g, function(
      match,
      emojiName
    ) {
  ...
  if (markupAttributes.length > 0)
    line.markup =
      "<span " + markupAttributes.join(" ") + ">" + line.markup + "</span>";

  if (line.hasOwnProperty("bash")) {
    // Append BitBar's legacy "paramN" attributes to the bash command
    // (Argos allows placing arguments directory in the command string)
    let i = 1;
  ...
  for (let i = 0; i < tokens.length; i++) {
    if (regex.match(tokens[i], 0)[0]) {
      ...
      for (let j = 0; j < sgrCodes.length; j++) {
        if (sgrCodes[j].length === 0) continue;
      ...
      if (textToken.length > 0) {
        ...
        for (let attribute in markupAttributes) {
          attributeString +=
            " " + attribute + "='" + markupAttributes[attribute] + "'";
        }
...
function execute(button, line) {
  ...
  if (line.hasOwnProperty("bash")) {
    ...
    if (line.terminal === "false") {
      ...
      let terminal = getSettings().get_string("terminal");
      if (terminal == null) {
      ...
      argv = [
        terminal,
        "-e",
        "bash -c " + GLib.shell_quote(line.bash + "; exec bash")
      ];
    }
    ...
    if (success) {
      GLib.child_watch_add(GLib.PRIORITY_DEFAULT_IDLE, pid, function() {
        if (line.refresh === "true") button.update(false, false);
      });
    }
  } else if (line.refresh === "true") {
    button.update(false, false);
  }
}

// https://gitlab.gnome.org/GNOME/gnome-shell-extensions/blob/master/lib/convenience.js
